...
//			customerActiveBookingDTO.setCurrentLocation(booking.getVehicle().getCurrentCity());
//	    } else {
//	    	customerActiveBookingDTO.setCurrentLocation("Location Unknown");
//	    }
//
...
//	}
	public ResponseEntity<ResponseStructure<CustomerActiveBookingDTO>> CustomerSeeActiveBooking(long mobileNo) {
	    ...
	    try {
	        // 1. Find Customer safely
	        Optional<Customer> customerOpt = customerRepo.findByMobileNo(mobileNo);
	        ...
	        // 2. Fetch Active Booking using the JPQL method
	        List<Booking> bookings = bookingRepo.findActiveByMobile(mobileNo);

	        if (bookings == null || bookings.isEmpty()) {
	            // No booking? Reset flag and return 200 with null data
	            customer.setBookingflag(false);
	            customerRepo.save(customer);
	            
	            rs.setStatuscode(200);
	            rs.setMessage("No current booking found");
	            rs.setData(null);
	            return new ResponseEntity<>(rs, HttpStatus.OK);
	        }
	        ...
	        // 3. Map to Flattened DTO
	        CustomerActiveBookingDTO dto = new CustomerActiveBookingDTO();
	        dto.setCustomername(customer.getName());
	        dto.setCustomerMobile(customer.getMobileNo());
	        
	        // Flattening the vehicle data prevents React 'undefined' crashes
	        if (booking.getVehicle() != null) {
	            dto.setVehicleName(booking.getVehicle().getName());
	            dto.setVehicleNumber(booking.getVehicle().getVehicleNo());
	            dto.setCurrentLocation(booking.getVehicle().getCurrentCity());
	        } else {
	            dto.setVehicleName("Assigning...");
	            dto.setVehicleNumber("Pending");
	            dto.setCurrentLocation("Calculating...");
	        }
...
//--------------------------------------------------------------------------------
	...
	public ResponseStructure<List<Booking>> getCustomerHistory(long mobileNo) {
	    // Directly fetch using the long mobileNo
	    List<Booking> history = bookingRepo.findByCustomerMobileNo(mobileNo);

	    ResponseStructure<List<Booking>> rs = new ResponseStructure<>();
	    rs.setStatuscode(HttpStatus.OK.value());
	    rs.setMessage("History fetched successfully");
	    rs.setData(history);
